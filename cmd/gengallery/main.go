...
func main() {
	...
	statsPath := filepath.Join(root, "website", "data", "catalog.yaml")
	...
	for _, c := range cat.Countries {
		tm, _ := c["tiers"].(map[string]any)
		for name := range tm {
			tiers[name] = struct{}{}
		}
	}
	writeCatalogStats(statsPath, cat.GeneratedAt, cat.Source, len(cat.Countries), len(cat.Destinations), len(tiers))
...
func writeCatalogStats(path, generatedAt, source string, countries, destinations, tiers int) {
	var b strings.Builder
	b.WriteString("# Generated by cmd/gengallery — do not edit by hand.\n")
	fmt.Fprintf(&b, "countries: %d\n", countries)
	fmt.Fprintf(&b, "destinations: %d\n", destinations)
	fmt.Fprintf(&b, "tiers: %d\n", tiers)
	fmt.Fprintf(&b, "generatedAt: %q\n", generatedAt)
